...
// メール等の成果物に紛れるマークダウン装飾(** や 見出しの #)を除去し、プレーンな文章にする。
...
export async function POST(req: NextRequest) {
  ...
  const execMessages =
    mode === "revise" && ex?.given
      ? {
          system:
            "あなたは有能な日本語の業務アシスタントです。『元の出力』を、ユーザーの『修正指示』に従って改善版に書き直してください。改善版の本文だけを返し、前置き・説明は不要です。装飾記号(** や # などのマークダウン)は使わず、そのまま貼って使えるプレーンな文章で出力してください。",
          content: `【元の出力】\n${ex.given}\n\n【修正指示】\n${instruction}`,
        }
      : {
          system:
            "あなたは有能な日本語の業務アシスタントです。ユーザーの指示に忠実に従い、成果物だけを出力してください。前置き・言い訳・補足説明は不要です。装飾記号(** や # などのマークダウン)は使わず、そのまま貼って使えるプレーンな文章で出力してください。",
          content: instruction,
        };
